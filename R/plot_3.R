#' plot_3 function
#'
#' Generate plot case 3 : everything is present
#'
...
#' @examples
#' #...
plot_3 <- function(seqs, genes, links, region, elegans_file){
  ...
  p2 <- gggenomes(seqs = seqs, genes = elegans_genes, links = links) +
    geom_seq() +
  ...
  combined_legend <- plot_grid(legend1, legend2, ncol = 1, nrow = 2, align = "v", rel_heights = c(1, 1))
