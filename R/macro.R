#' macro function
#'
#' Transform aligned genes data to macrosynteny data
#'
#' Belong to the server function in the macrosynteny button section
#'
...
#'
#' @return An aligned genes data frame with gene gene type
#'
#' @importFrom dplyr %>%
...
#' @examples
#' #...
macro <- function(genes, seqs, species){
  ...
  for(i in 1:11){
    ...
    #Select seqs from one species
    seqs_species <- seqs %>% filter(str_extract(seq_id, "[:alpha:]+") %in% spe[i])
    ...
    if(nrow(genes_species)==0){
      ...
      #If there is genes for that species > identify them and store them
    }else{
      #Identifying
      genes_species <- subset_by_overlap(species_macro[[i]], genes_species)
      genes_species <- genes_fct(genes_species, seqs_species)
      genes_species <- metadata_fct(species_macro[[i]], genes_species)
      genes_species <- check_overlapping(species_macro[[i]], genes_species)
      genes_species <- add_seq_id(genes_species, spe[i])
